...
#There were some errors with the original code leading to nans, so this removes the nans
...
#this model is the function that computes the implied volatility from the 15 inputs
...
model = nn.Sequential(nn.Linear(15, 256), nn.ReLU(), nn.Linear(256, 128), nn.ReLU(), nn.Linear(128, 32), nn.ReLU(), nn.Linear(32,1)).to(device)
...
num_epochs = 200      #number of learning cycles, can vary this in the future
